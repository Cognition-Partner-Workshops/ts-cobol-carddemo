...
{
    private static (TransactionViewService Service, InMemoryTransactionRepository Repository) Build()
    {
        var repository = new InMemoryTransactionRepository();
        repository.Add(InMemoryTransactionRepository.SampleTransaction());
    ...
    public async Task Id_IsUsedVerbatimWithoutCaseFoldingOrLeadingBlankRemoval(string typed, string stored)
    {
        // FR-S08-12
        var repository = new InMemoryTransactionRepository();
        repository.Add(InMemoryTransactionRepository.SampleTransaction(stored));
