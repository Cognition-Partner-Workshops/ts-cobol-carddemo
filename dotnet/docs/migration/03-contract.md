...
| SEM-031 | CVTRA07Y edited pictures `-ZZZ,ZZZ,ZZZ.ZZ` / `+ZZZ,ZZZ,ZZZ.ZZ`: fixed leading sign position, full zero suppression (zero value edits to all spaces, suppressed commas become spaces, suppression stops at the decimal point) | Ported as `EditedPic` with identical semantics | EditedPicTests + tran-report goldens (byte-exact) |
| SEM-032 | CBTRN03C numeric→alphanumeric MOVEs onto detail line: `XREF-ACCT-ID` 9(11)→X(11) and `TRAN-CAT-CD` 9(04)→9(04) keep zero-padded digit images; alphanumeric X(50)→X(15)/X(29) MOVEs left-truncate descriptions | Ported via `ReportLayout.Detail` (D11/D4 + `Fit`) | TranReportParityTests (byte-exact) |
| SEM-033 | CBTRN03C page breaks: `FUNCTION MOD(WS-LINE-COUNTER, 20)` where the COMP-3 counter counts every written line (headers, dashes, totals, details), so pages drift from exactly 20 details after totals lines | Ported with identical counter increments | tran-report-eof golden (multi-page, byte-exact) |
| SEM-034 | CBTRN03C lookup misses (CARDXREF/TRANTYPE/TRANCATG INVALID KEY) abend via CEE3ABD | Port throws `InvalidDataException` (job host exits 99); never silently skips | N/A (not reachable from committed fixtures; goldens contain no miss) |
| SEM-B03 | CBTRN03C clean-EOF path re-reads the stale last TRAN-RECORD (record area unchanged at EOF), re-adds its amount to page/account totals before writing final page + grand totals, and never flushes the last account total | **Bug-for-bug preserved** | tran-report-eof golden (grand total includes double-count), TranReportParityTests |
