...
| SEM-031 | DISCGRP missing-key fallback (CBACT04C 1200-A-GET-DEFAULT-INT-RATE): file status 23 on the account's group retries with group 'DEFAULT'; a zero rate writes no transaction but the account is still rewritten (with cycle reset) on account break | Preserved in `InterestCalcJob.GetInterestRate` / account-break rewrite | InterestCalcJobTests.MissingDisclosureGroup_FallsBackToDefaultGroupRate, ZeroRate_WritesNoTransaction_ButAccountStillRewrittenOnBreak |
| SEM-032 | XREFFILE alternate-key READ (CBACT04C 1110-GET-XREF-DATA, `READ ... KEY IS FD-XREF-ACCT-ID`): returns the first record for the account in primary (card-number) key order; seed data maps one card per account (see SEM-024) | Preserved in `InterestCalcJob.FirstXrefPerAccount` | InterestCalcJobTests.XrefAlternateKeyRead_PicksFirstCardInPrimaryKeyOrder |
