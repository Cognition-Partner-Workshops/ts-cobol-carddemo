...
> Java-engagement note (2026-09-15): this inventory was produced under the prior (.NET)
> engagement and verified still accurate against `main` for this run — it is source-side
> analysis (call graph, entry points, coverage, boundaries), which does not change with the
> target stack. Re-presented at this engagement's STOP B.
