...
> Test column and §6/§9/§11 target references were re-expressed for Java 21/Spring Boot
...
|---|---|---|---|---|---|---|---|
| FR-S06-01 | Screen | First entry from the menu (or no COMMAREA) | Empty screen, search fields editable, detail fields protected, info `Please enter Account and Card Number`, cursor on account | COCRDUPC | `:504-509`, `:1145-1147`, `:1172-1178` | S06-B3 | `CardUpdateServiceTest` + `CardUpdateUiIntegrationTest` |
| FR-S06-02 | AID | Any key other than ENTER, PF3, PF5 (state N only), PF12 (details fetched only) | Treated exactly as ENTER; no invalid-key message exists in this program | COCRDUPC | `:413-424` | — | `CardUpdateServiceTest` + `CardUpdateUiIntegrationTest` |
| FR-S06-03 | Navigation | PF3 in any state | Transfer to the calling program (menu) — target route `/menu` | COCRDUPC | `:435-478` | S06-B3 | `CardUpdateUiIntegrationTest` |
| FR-S06-04 | Search validation | ENTER while not fetched, account blank/zero/`*` | Error `Account number not provided`, account flagged (shown as `*`), cursor on account | COCRDUPC | `:721-738`, `:1245-1250` | — | `CardUpdateServiceTest` |
| FR-S06-05 | Search validation | Account not exactly 11 digits (shorter values are space-padded and fail `IS NUMERIC`) | Error `ACCOUNT FILTER,IF SUPPLIED MUST BE A 11 DIGIT NUMBER`, account flagged | COCRDUPC | `:740-748` | — | `CardUpdateServiceTest` |
| FR-S06-06 | Search validation | Card blank/zero/`*` | Error `Card number not provided` (only if no earlier message), card flagged | COCRDUPC | `:762-779` | — | `CardUpdateServiceTest` |
| FR-S06-07 | Search validation | Card not exactly 16 digits | Error `CARD ID FILTER,IF SUPPLIED MUST BE A 16 DIGIT NUMBER` | COCRDUPC | `:784-792` | — | `CardUpdateServiceTest` |
| FR-S06-08 | Search validation | Both search fields blank | Error `No input received` overrides the individual blank messages | COCRDUPC | `:657-663` | — | `CardUpdateServiceTest` |
| FR-S06-09 | Search validation | Account fails and card fails | Only the first (account) message is shown; both fields flagged; cursor on account | COCRDUPC | `:732`, `:776`, `:1232-1244` | — | `CardUpdateServiceTest` |
| FR-S06-10 | Lookup | Valid keys, card number exists | Card read by card number only (account not matched); OLD image captured with upper-cased name; state S; details editable, search fields protected; info `Details of selected card shown above`; cursor on name | COCRDUPC | `:951-957`, `:1343-1412`, `:1354-1358` | S06-B1 | `CardUpdateServiceTest` + `CardUpdateIntegrationTest` |
| FR-S06-11 | Lookup | Valid keys, card not found | Error `Did not find cards for this search condition`; both search fields flagged; stays not fetched | COCRDUPC | `:1395-1400` | S06-B1 | `CardUpdateServiceTest` + `CardUpdateIntegrationTest` |
| FR-S06-12 | Lookup | Other read failure | Error `File Error: READ     on CARDDAT   returned RESP nnnnnnnnn ,RESP2 nnnnnnnnn ` (75-char template); both search fields flagged | COCRDUPC | `:146-158`, `:1401-1409` | S06-B1 | `CardUpdateServiceTest` |
| FR-S06-13 | Edit | ENTER in S/E, no field differs from OLD (case-insensitive over name, year, month, day, status) | Error `No change detected with respect to values fetched.`; OLD image redisplayed; state stays S | COCRDUPC | `:680-688`, `:967-975` | — | `CardUpdateServiceTest` |
| FR-S06-14 | Edit | Name blank | Error `Card name not provided`; name flagged / `*` | COCRDUPC | `:806-823` | — | `CardUpdateServiceTest` |
| FR-S06-15 | Edit | Name contains anything other than letters and spaces | Error `Card name can only contain alphabets and spaces` | COCRDUPC | `:825-838` | — | `CardUpdateServiceTest` |
| FR-S06-16 | Edit | Status blank or not `Y`/`N` (upper case only) | Error `Card Active Status must be Y or N` | COCRDUPC | `:91`, `:845-870` | — | `CardUpdateServiceTest` |
| FR-S06-17 | Edit | Month blank, non-numeric or not 01..12 (single digit is zero-filled by BMS JUSTIFY=RIGHT) | Error `Card expiry month must be between 1 and 12` | COCRDUPC | `:95`, `:877-908`; `bms:129` | — | `CardUpdateServiceTest` |
| FR-S06-18 | Edit | Year blank, non-numeric or not 1950..2099 (zero-filled, so `25` → `0025` fails) | Error `Invalid card expiry year` | COCRDUPC | `:99`, `:913-943`; `bms:137` | — | `CardUpdateServiceTest` |
| FR-S06-19 | Edit | Several fields invalid | All four edits run; first message in order name → status → month → year is shown; all failing fields flagged; cursor on the first failing field; state E; info `Update card details presented above.`; NEW values redisplayed | COCRDUPC | `:690-712`, `:1153-1154`, `:1222-1244` | — | `CardUpdateServiceTest` |
| FR-S06-20 | Confirm | All edits pass | State N; everything protected; info `Changes validated.Press F5 to save`; `F5=Save F12=Cancel` legend | COCRDUPC | `:706-712`, `:1155-1156`, `:1310-1316` | — | `CardUpdateServiceTest` + `CardUpdateUiIntegrationTest` |
| FR-S06-21 | Confirm | ENTER while in N | Screen redisplayed unchanged in N | COCRDUPC | `:685-688`, `:1015-1020` | — | `CardUpdateServiceTest` |
| FR-S06-22 | Save | PF5 in N, record locked, unchanged since fetch | Rewrite name (as typed), status, `year-month-<old day>`; state C; info `Changes committed to database`; everything protected | COCRDUPC | `:985-1013`, `:1420-1495` | S06-B1 | `CardUpdateServiceTest` + `CardUpdateIntegrationTest` |
| FR-S06-23 | Save | PF5 in N, record differs from OLD image (name case-insensitive, year, month, day, status) | Error `Record changed by some one else. Please review`; OLD image refreshed from the current record; state S | COCRDUPC | `:1449-1454`, `:1498-1519` | S06-B1 | `CardUpdateServiceTest` + `CardUpdateIntegrationTest` |
| FR-S06-24 | Save | READ UPDATE fails (record gone / store error) | Error `Could not lock record for update`; state L; info `Changes unsuccessful. Please try again` | COCRDUPC | `:1436-1447` | S06-B1 | `CardUpdateServiceTest` + `CardUpdateIntegrationTest` |
| FR-S06-25 | Save | REWRITE fails | Error `Update of record failed`; state F; info `Changes unsuccessful. Please try again` | COCRDUPC | `:1487-1491` | S06-B1 | `CardUpdateServiceTest` |
| FR-S06-26 | Reset | ENTER after state C, L or F (caller = menu) | Everything re-initialised; fresh search screen as FR-S06-01 | COCRDUPC | `:517-528` | — | `CardUpdateServiceTest` + `CardUpdateUiIntegrationTest` |
| FR-S06-27 | Cancel | PF12 with details fetched | Record re-read; OLD image redisplayed in state S; message produced by the preceding edit pass (if any) is kept | COCRDUPC | `:958-965`, `:669-712` | S06-B1 | `CardUpdateServiceTest` |
| FR-S06-28 | Entry from list | Called with account + card already known | Record read immediately and shown in state S (target: `/cards/update?acctId=&cardNum=`) | COCRDUPC | `:510-516`, `COCRDLIC.cbl:204-206` | S06-B2 | `CardUpdateUiIntegrationTest` |
| FR-S06-29 | Screen | Field lengths | Inputs limited to 11 / 16 / 50 / 1 / 2 / 4 characters; day is read-only | COCRDUPC | `bms:84-146` | — | `CardUpdateUiIntegrationTest` |
...
- Rewritten date: `STRING year '-' month '-' day` (`:1466-1472`); target composes `LocalDate.of(year, month, oldDay)`.
...
| FR | Service/API | UI |
|---|---|---|
| 01, 26 | `CardService` fresh-search path | `templates/card-update.html` initial/reset render via `UiController` |
| 02 | `CardService` AID normalisation (unmapped → ENTER, source-faithful) | `aid` form field classified in `UiController` |
| 03, 28, 29 | — | `UiController` routes `GET/POST /cards/update` (+ `?acctId=&cardNum=` pre-fetch) |
| 04–09 | `CardService` search edits | message area + field flagging |
| 10–12 | `CardService` read + `CardRepository.findById` | display |
| 13–21 | `CardService` change edits (name/status/month/year order) | display / attributes |
| 22–25 | `CardService.save` + locked read/`save` in `@Transactional` (`PUT /api/cards/{cardNumber}`) | display |
| 27 | `CardService` cancel path (re-read) | F12 via `aid` field |

Test homes: `spring-boot/src/test/java/com/carddemo/service/CardServiceTest.java` (or `CardUpdateServiceTest` if the wave splits a dedicated class), `CardUpdateIntegrationTest.java` (Testcontainers), `CardUpdateUiIntegrationTest.java` (MockMvc).
...
- **D3 (storage)**: the expiry date is a `date` column (`LocalDate`); a month/day combination that is not a calendar date (e.g. day 31 with month 02) is reported through the REWRITE-failure path FR-S06-25 instead of being stored as text.
