...
@SpringBootTest(
    properties = {
      "carddemo.loader.enabled=true",
      "carddemo.loader.data-directory=../../app/data/ASCII",
      "spring.datasource.url=jdbc:h2:mem:carddemo;MODE=PostgreSQL;DB_CLOSE_DELAY=-1"
    })
class AsciiDataLoaderIntegrationTest {
  ...
  @Autowired TransactionCategoryBalanceRepository balances;

  @Test
  void loadsAsciiSamplesAndJoinsCardAccountCustomer() {
    ...
    CardXref x = xrefs.findByXrefCardNum("0500024453765740");
    assertThat(x.getAccount().getAcctId()).isEqualTo(50L);
    assertThat(x.getCustomer().getCustId()).isEqualTo(50L);
  }
}
