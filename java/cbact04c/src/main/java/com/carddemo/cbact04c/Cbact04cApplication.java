...
@SpringBootApplication public class Cbact04cApplication {
 ...
 @Bean CommandLineRunner runner(){return a->{if(a.length<6)throw new AbendException("Usage: <tcatbal> <xref> <discgrp> <acctdata> <transact> <parm-date> [final-update-at-eof]");boolean finalUpdate=a.length>6?Boolean.parseBoolean(a[6]):Boolean.parseBoolean(System.getProperty("cbact04c.final-update-at-eof","false"));BatchResult r=new Cbact04cService(java.time.Clock.systemDefaultZone(),finalUpdate).run(Paths.get(a[0]),Paths.get(a[1]),Paths.get(a[2]),Paths.get(a[3]),Paths.get(a[4]),a[5]);System.out.println("RECORD COUNT: "+r.recordCount());System.out.println("TRANSACTIONS GENERATED: "+r.transactionCount());};}
 @Bean ExitCodeExceptionMapper exitCodeExceptionMapper(){return e->e instanceof AbendException||e.getCause() instanceof AbendException?999:1;}
